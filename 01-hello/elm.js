...
Elm.Main.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var name = $Signal.mailbox("");
   var sayHello = function (name) {    return $String.isEmpty(name) ? "Who are you?" : A2($Basics._op["++"],"Hello",A2($Basics._op["++"]," ",name));};
   var viewBuilder = F2(function (address,message) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.placeholder("Please enter your name")
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (v) {    return A2($Signal.message,address,v);})]),
              _U.list([]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(sayHello(message))]))]));
   });
   var main = A2($Signal.map,viewBuilder(name.address),name.signal);
   return _elm.Main.values = {_op: _op,sayHello: sayHello,viewBuilder: viewBuilder,name: name,main: main};
};
